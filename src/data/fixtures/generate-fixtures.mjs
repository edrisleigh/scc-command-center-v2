...
// Generate all dates from 2025-09-01 to 2025-11-30
...
const shopDaily = dates.map((date) => {
  ...
  const affiliateGmvPct = 1 - liveGmvPct - videoGmvPct - productCardGmvPct
...
const videoDaily = dates.map((date) => {
  const dayOfWeek = new Date(date).getDay()
  const isWeekend = dayOfWeek === 0 || dayOfWeek === 6
  ...
  const gpm = roundTo((videoGmv / videoViews) * 1000, 4) // GMV per 1000 views
...
// Write files
writeFileSync(
  join(__dirname, 'shop-daily.json'),
  JSON.stringify(shopDaily, null, 2),
)
console.log(`Generated shop-daily.json: ${shopDaily.length} records`)

writeFileSync(
  join(__dirname, 'video-daily.json'),
  JSON.stringify(videoDaily, null, 2),
)
console.log(`Generated video-daily.json: ${videoDaily.length} records`)

writeFileSync(
  join(__dirname, 'ads-daily.json'),
  JSON.stringify(adsDaily, null, 2),
)
