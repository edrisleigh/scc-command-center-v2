...
function aggregateData(
  ...
  return Array.from(groups.entries()).map(([key, rows]) => {
    ...
    if (secondaryDataKey) {
      result[secondaryDataKey] = rows.reduce((sum, r) => sum + (r[secondaryDataKey] as number), 0)
    }
    return result
  })
}
...
export function TimeSeriesChart({
  title, data, dataKey, secondaryDataKey, xAxisKey,
  color = '#a78bfa', secondaryColor = '#60a5fa',
  valueFormatter = (v) => v.toLocaleString(),
}: TimeSeriesChartProps) {
  const [granularity, setGranularity] = useState<Granularity>('daily')
  const chartData = aggregateData(data, xAxisKey, dataKey, secondaryDataKey, granularity)
