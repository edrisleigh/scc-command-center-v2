...
function ClientCard({ name, slug, orgSlug, gmv, orders, activeCreators, roas, loading }: ClientCardProps) {
  return (
    <div className="rounded-xl border border-border bg-card p-6 flex flex-col gap-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-base font-bold text-card-foreground">{name}</span>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Metric label="Active Creators" value={formatNumber(activeCreators)} />
...
function AggKpiRow({ totalGmv, totalOrders, totalCreators, avgRoas }: AggKpiRowProps) {
  return (
...
function AgencyOverviewPage() {
  ...
  const heydudeCreators = heydudeData ? (heydudeData.creators as Creator[]).filter((c) => c.p28dAffiliateGmv > 0).length : 0
